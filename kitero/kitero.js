...
function hideBasedOnSelected(dropdownList){
    ...
    const parent = dropdownList.parentElement // elkerjuk a parameter value parentElementjet
    const cardClassList = parent.querySelectorAll(".card") // elkerjuk a card osztallyal rendelkezo htmlElement parentElementjet
    for (const x of cardClassList ){ // vegigiteralunk a listan 
        x.classList.add("hide")  // minden elemre ratesszuk a hideot
    }
    makeVisibleBasedOnId(parent, value) // makeVisibleBasedOnId(parentElement, value)
}
